...
describe("LiabilityPreset", function () {
  let liabilityPreset;
  let owner, publisher, worker, insuranceProvider;
  ...
  beforeEach(async function () {
    [owner, publisher, worker, insuranceProvider] = await ethers.getSigners();
  ...
  describe("Preset Creation", function () {
    it("Should create Standard preset", async function () {
      ...
      expect(preset.model).to.equal(0); // Standard
    ...
    it("Should create Limited preset", async function () {
      ...
      expect(preset.model).to.equal(1); // Limited
    ...
    it("Should create Insured preset", async function () {
      ...
      expect(preset.model).to.equal(2); // Insured
    ...
    it("Should create Bonded preset", async function () {
      ...
      expect(preset.model).to.equal(3); // Bonded
      expect(preset.publisherLiability).to.equal(ethers.parseEther("0.1"));
  ...
  describe("Apply Preset", function () {
    it("Should apply Standard preset without payment", async function () {
      await liabilityPreset.connect(publisher).applyPreset(1, STANDARD_PRESET);
    ...
    it("Should apply Limited preset with publisher liability", async function () {
      await expect(
        liabilityPreset.connect(publisher).applyPreset(1, LIMITED_PRESET)
      ).to.be.revertedWith("Insufficient liability deposit");

      await liabilityPreset.connect(publisher).applyPreset(1, LIMITED_PRESET, {
    ...
    it("Should apply Insured preset with insurance premium", async function () {
      ...
      await liabilityPreset.connect(publisher).applyPreset(1, INSURED_PRESET, {
        value: ethers.parseEther("0.11") // 0.1 liability + 0.01 premium
      });
    ...
    it("Should refund excess payment", async function () {
      ...
      const tx = await liabilityPreset.connect(publisher).applyPreset(1, LIMITED_PRESET, {
        value: ethers.parseEther("0.1") // Sending more than required
      });
      const receipt = await tx.wait();
      const gasCost = receipt.gasUsed * receipt.gasPrice;

      const balanceAfter = await ethers.provider.getBalance(publisher.address);
      const expectedBalance = balanceBefore - ethers.parseEther("0.05") - gasCost;
  ...
  describe("Worker Liability", function () {
    it("Should require worker deposit for Bonded preset", async function () {
      await liabilityPreset.connect(publisher).applyPreset(1, BONDED_PRESET, {
    ...
    it("Should not accept deposit for non-Bonded preset", async function () {
      ...
      await expect(
        liabilityPreset.connect(worker).depositWorkerLiability(1, {
  ...
  describe("Release Liability", function () {
    it("Should release liability on task completion", async function () {
      ...
      await liabilityPreset.releaseLiability(1, publisher.address, worker.address);
  ...
  describe("Slash Liability", function () {
    it("Should slash liability for disputed party", async function () {
      ...
      await liabilityPreset.slashLiability(1, publisher.address, owner.address);
